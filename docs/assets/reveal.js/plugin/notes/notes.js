...
</html>`),!t)return void alert("Speaker view popup failed to open. Please make sure popups are allowed and reopen the speaker view.");!function(){const n=e.getConfig().url,s="string"==typeof n?n:window.location.protocol+"//"+window.location.host+window.location.pathname+window.location.search;i=setInterval(function(){t.postMessage(JSON.stringify({namespace:"reveal-notes",type:"connect",state:e.getState(),url:s}),"*")},500),window.addEventListener("message",a)}()}}function s(){let o=e.getCurrentSlide(),s=o.querySelectorAll("aside.notes"),i=o.querySelector(".current-fragment"),n={namespace:"reveal-notes",type:"state",notes:"",markdown:!1,whitespace:"normal",state:e.getState()};if(o.hasAttribute("data-notes")&&(n.notes=o.getAttribute("data-notes"),n.whitespace="pre-wrap"),i){let e=i.querySelector("aside.notes");e?(n.notes=e.innerHTML,n.markdown="string"==typeof e.getAttribute("data-markdown"),s=null):i.hasAttribute("data-notes")&&(n.notes=i.getAttribute("data-notes"),n.whitespace="pre-wrap",s=null)}s&&(n.notes=Array.from(s).map(e=>e.innerHTML).join(`
`),n.markdown=s[0]&&"string"==typeof s[0].getAttribute("data-markdown")),t.postMessage(JSON.stringify(n),"*")}function a(n){if(function(e){try{return window.location.origin===e.source.location.origin}catch{return!1}}(n)){let s=JSON.parse(n.data);s&&"reveal-notes"===s.namespace&&"connected"===s.type?(clearInterval(i),r()):s&&"reveal-notes"===s.namespace&&"call"===s.type&&function(n,s,o){let i=e[n].apply(e,s);t.postMessage(JSON.stringify({namespace:"reveal-notes",type:"return",result:i,callId:o}),"*")}(s.methodName,s.arguments,s.callId)}}function r(){e.on("slidechanged",s),e.on("fragmentshown",s),e.on("fragmenthidden",s),e.on("overviewhidden",s),e.on("overviewshown",s),e.on("paused",s),e.on("resumed",s),s()}return{id:"notes",init:function(n){e=n,/receiver/i.test(window.location.search)||(null!==window.location.search.match(/(\?|&)notes/gi)?o():window.addEventListener("message",e=>{if(!t&&"string"==typeof e.data){let s;try{s=JSON.parse(e.data)}catch{}s&&"reveal-notes"===s.namespace&&"heartbeat"===s.type&&(n=e.source,t&&!t.closed?t.focus():(t=n,window.addEventListener("message",a),r()))}var n}),e.addKeyBinding({keyCode:83,key:"S",description:"Speaker notes view"},function(){o()}))},open:o}}})